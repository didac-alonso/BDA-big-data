...
def check_following_days(DATA):
    ...
    for i in range(1,8):
        ...
        # We check if the i-following day:
        # - In that day is already labeled as following_uOI
        # - If the time between the i-following day and the timeID is up to 7 days and there is an unscheduledOI in that day with a  has an unscheduledOI, if it is not, we set the unscheduledOI column to 0
        # - In a previous iteration label == 1 has been imputed 
        DATA = DATA.withColumn('label', F.when(((F.col('following_uOI') == 1)&(F.date_add(F.col('timeid'),7) > F.col('following_day')))\
                    |(F.col('following_uOI') == 1)|(F.col('label') == 1),1).otherwise(0))
...
def p1Management(spark,AMOS,DW):
    ...
    # We filter the data to keep only the subsystem 3453 and the unscheduled operation interruptions. We drop the columns we do not use any more (kind and subsystem)
    ...
    # We join the data from operation interruption with the data from the sensors+KPIs, we perform a full join to keep all the data from the sensors, and add 0 
    # for the (aircraft,time) that don't have any interruption (in the "unscheduledOI" ). 
    # We also need to keep all the DATA from operation interruption, because we need to check if there is an unscheduled operation interruption during the 7 following days
    ...
    # We add the label column which will be imputed in the check following_days --> 
